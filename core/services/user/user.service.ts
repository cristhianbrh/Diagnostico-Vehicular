...
export class UserService implements IUserService {
  public async getCurrentUser(): Promise<UserResponse> {
    try {
      ...
      } = await axios.get<UserResponse>(`${process.env.API_URL}/api/auth/me`, {
  ...
  public async getUserName(userId: number): Promise<ApiResponse<string>> {
    try {
      const { name: data } = await prisma.user.findUniqueOrThrow({
        where: { id: userId },
      });
      return { data };
    } catch (error: any) {
  ...
  public async getUsers(listParams?: {
    limit?: number;
    offset?: number;
  ...
  public async getAllUsers(): Promise<ApiResponse<UserSummary[]>> {
    try {
      const { data: response } = await axios.get<ApiResponse<UserSummary[]>>(
        `/api/users/getUsers`
      );
  ...
  public async updateUser(
    userId: number,
    userData: UserSummaryEdit
  ): Promise<UserResponse> {
    try {
      const {
        data: { data: user, error },
      } = await axios.put<UserResponse>(
        `${process.env.API_URL}/api/users/updateUser`,
        {
          id: userId,
          ...userData,
        }
